...
GLOBALDEF int FtpConnect(const char *host, netbuf **nControl) {
  ...
  pnum = strchr(lhost, ':');
  *pnum++ = '\0';
  if (isdigit(*pnum)) sin.sin_port = htons((short)atoi(pnum));
  /*
else
{
...
static int FtpOpenPort(netbuf *nControl, netbuf **nData, int mode, int dir) {
  ...
  if (nControl->cmode == FTPLIB_PASSIVE) {
    ...
    sprintf(buf, "PORT %d,%d,%d,%d,%d,%d", (unsigned char)sin.sa.sa_data[2],
            (unsigned char)sin.sa.sa_data[3], (unsigned char)sin.sa.sa_data[4],
            (unsigned char)sin.sa.sa_data[5], (unsigned char)sin.sa.sa_data[0],
...
GLOBALDEF int FtpAccess(const char *path, int typ, int mode, netbuf *nControl,
  ...
  if (FtpOpenPort(nControl, nData, mode, dir) == -1) return 0;
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpDir(const char *outputfile, const char *path,
                     netbuf *nControl) {
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpSize(const char *path, int *size, char mode,
                      netbuf *nControl) {
  char cmd[256];
  int resp, sz, rv = 1;
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpModDate(const char *path, char *dt, int max,
                         netbuf *nControl) {
  char buf[256];
  int rv = 1;
 ...
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpGet(const char *outputfile, const char *path, char mode,
                     netbuf *nControl) {
...
// --------------------------
// New functions for the DS. They do essentially the same as the old functions,
// but will
//  not dump data into a file, but into a buffer instead.
//
/*
 * FtpDir - issue a LIST command and write response to output
 *
 * return 1 if successful, 0 otherwise
 */
static int FtpXferBuf(char *buffer, int size, const char *path,
  ...
  if (typ == FTPLIB_FILE_WRITE) {
    // while ((l = fread(buffer, 1, size, local)) > 0)
    if ((c = FtpWrite(buffer, size, nData)) < l) {
      printf("short write: passed %d, wrote %d\n", l, c);
      rv = 0;
      // break;
    }
  } else {
    while ((l = FtpRead(buffer, size, nData)) > 0) {
      buffer += l;
      size -= l;
    }
    /*
...
GLOBALDEF int FtpDirBuf(char *buffer, int size, const char *path,
                        netbuf *nControl) {
  return FtpXferBuf(buffer, size, path, nControl, FTPLIB_DIR_VERBOSE,
                    FTPLIB_ASCII);
}

GLOBALDEF int FtpAccessBuf(const char *path, int typ, int mode,
...
/*
 * FtpAccessPos - return a handle for a data stream, starting at some offset
 * "pos"
 *
 * return 1 if successful, 0 otherwise
 */
GLOBALDEF int FtpAccessPos(const char *path, int typ, int mode, int pos,
                           netbuf *nControl, netbuf **nData) {
  char buf[256];
  int dir;
  if ((path == NULL) &&
