/*  
	...
	MirageIslandValue v1.5 - Tool to find the specific Mirage Island Value in RSE save files
    Copyright (C) 2015  BlackShark
...
int wc_inject (char* sav, char* wc3, int game, int language) // game 0=RS, 1=E, 2=FRLG
{
	...
	if (language == 1)
	{
		memcpy(sav + ( wc_offset + 0x1000 * sec[4] + currentSav), wc3, 0x4 + 0xA4); // checksum+WC
		memcpy(sav + ( (wc_offset + 0x4 + 0xA4 + 0xA) + 0x1000 * sec[4] + currentSav), wc3 + 0x4 + 0xA4 + 0xA, 2); // Icon
		memcpy(sav + ( wc_script_offset + 0x1000 * sec[4] + currentSav), wc3 + 0x4 + 0xA4 + 0x28 + 0x28, 1004); // Script data (chk(4) + association(4) + script(996))
	}
	else
	{
	
		memcpy(sav + ( wc_offset + 0x1000 * sec[4] + currentSav), wc3, 0x4 + 0x14C); // checksum+WC
		memcpy(sav + ( (wc_offset + 0x4 + 0x14C + 0xA) + 0x1000 * sec[4] + currentSav), wc3 + 0x4 + 0x14C + 0xA, 2); // Icon
		memcpy(sav + ( wc_script_offset + 0x1000 * sec[4] + currentSav), wc3 + 0x4 + 0x14C + 0x28 + 0x28, 1004); // Script data (chk(4) + association(4) + script(996))
	}
...
int me_inject (char* sav, char* me3, int game, int language) // game 0=RS, 1=E, 2=FRLG
{

	if (me3 == NULL && language != 0 && game != 1) //Only allow NULL card for emerald eon ticket JAP
		return -4;
	...
	else
	{
		...
		if (language == 1)
		{
			memcpy(sav + ( me_offset + 0x1000 * sec[4] + currentSav), me3, 1012); // Script data (chk(4) + association(4) + script(996)) + item data (8)
		}
		else
		{
			memcpy(sav + ( me_offset + 0x1000 * sec[4] + currentSav), me3, 1012); // Script data (chk(4) + association(4) + script(996)) + item data (8)
