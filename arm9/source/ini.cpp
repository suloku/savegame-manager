...
ini_t *__ini_open(const char *name, ini_mode_t mode, const char *comment,
  ...
  if (!ini) goto ini_openError;
  memset(ini, 0, sizeof(ini_t));

  // Store ini filename
  ini->filename = strdup(name);
  if (!ini->filename) goto ini_openError;
  ...
  if (ini->mode == INI_READ) {
    // this DOES NOT WORK with devkitpro! use a different approach instead!
    // ini->ftmp = tmpfile ();
    ini->ftmp = fopen("/tmpfile", "wb+");
  } else {
  ...
  if (!ini->ftmp) goto ini_openError;
  if (file) {  // Process existing ini file
    if (__ini_process(ini, file, comment) < 0) goto ini_openError;
    fclose(file);
  }
  return ini;
...
ini_openError:
  if (ini) {
    ...
    if (ini->filename) free(ini->filename);
    free(ini);
  }
...
int __ini_close(ini_t *ini, bool flush) {
  ...
  if (ini->tmpSection.heading) free(ini->tmpSection.heading);
  if (ini->tmpKey.key) free(ini->tmpKey.key);
...
void __ini_delete(ini_t *ini) {  // If already deleted, don't delete it again
  ...
  if (ini->mode != INI_READ) ini->flags |= INI_MODIFIED;
}
    ...
    case '=':
      if (parser.state !=
        ...
        if (parser.first >= 0) {
          ...
          {
            if (!__ini_faddHeading(ini, file, 0, 0)) return false;
          }

          parser.key = __ini_faddKey(ini, file, parser.first,
                                     parser.last - parser.first);
          if (!parser.key) return false;
        }
...
bool __ini_processComment(ini_t *ini, FILE *file, ini_parser_t &parser) {
  ...
  for (; parser.commentpos > 0; parser.commentpos--) {
    if (!__ini_extractField(ini, file, parser, *p++)) return false;
    parser.pos++;
  }
  return true;
}
    ...
    while (count--) {
      ...
      switch (ch) {
        ...
        __ini_processLineEnd:
          if (!__ini_processComment(ini, file, parser)) goto __ini_processError;
          parser.pos = pos;
      ...
      if (parser.state == INI_END_OF_FILE) break;
...
int __ini_store(ini_t *ini, FILE *file) {
  ...
  if (selected_h != NULL) selected_k = selected_h->selected;
  ...
  while (current_h) {
    ...
    while (current_k) {
      if (((current_k->length + 1) > length) || !str) {  // Need more space
        if (str) free(str);
        length = current_k->length + 1;
        str = (char *)malloc(sizeof(char) * length);
        if (!str) goto __ini_storeError;
      }
      ...
      {  // Output key
        ...
        if (fprintf(file, format, current_k->key) < 0) goto __ini_storeError;
      }

      // Output keys data (point to correct keys data)
      ini->selected = current_h;
      current_h->selected = current_k;
      if (ini_readString((ini_fd_t)ini, str, length) < 0) goto __ini_storeError;

      if (fprintf(file, "%s\n", str) < 0) goto __ini_storeError;

      current_k = current_k->pNext;
    }

    current_h = current_h->pNext;
    if (fprintf(file, "\n") < 0) goto __ini_storeError;
  }
  ret = 0;
...
__ini_storeError:
  ...
  if (selected_h != NULL) selected_h->selected = selected_k;
  return ret;
}
...
int INI_LINKAGE ini_close(ini_fd_t fd) {
  if (fd) return __ini_close((ini_t *)fd, true);
  return -1;
}
...
int INI_LINKAGE ini_flush(ini_fd_t fd) {
  if (fd) return __ini_close((ini_t *)fd, false);
  return -1;
}
...
#ifdef INI_ADD_LIST_SUPPORT
  if (ini->listDelims) return __ini_listIndexLength(ini);
#endif
...
extern "C" int INI_LINKAGE ini_append(ini_fd_t fddst, ini_fd_t fdsrc) {
  ...
  // Is file read only?
  if (dst->mode == INI_READ) return -1;

  // Backup selected heading and key
  src_h = src->selected;
  dst_h = dst->selected;
  // Be carefull if nothing was previously selected
  if (src_h != NULL) src_k = src_h->selected;
  if (dst_k != NULL) dst_k = dst_h->selected;
  ...
  while (current_h) {  // Locate heading in the dst
    ...
    while (current_k) {  // Check if data buffer can hold the key
      ...
      if (i > length) {  // Make data buffer bigger, with some spare
        ...
        if (data == NULL) goto ini_appendError;
      }
      // Locate key in dst ini file
      ini_locateKey(dst, current_k->key);
      // Copy the key from src to dst ini file
      if (ini_readString(src, data, length) != i) goto ini_appendError;
      if (ini_writeString(dst, data) < 0) goto ini_appendError;
      // Move to next key
      current_k = current_k->pNext;
    }
...
ini_appendError:
  ...
  src->selected = src_h;
  dst->selected = dst_h;
  if (src_h != NULL) src_h->selected = src_k;
  if (dst_h != NULL) dst_h->selected = dst_k;
  return ret;
}
